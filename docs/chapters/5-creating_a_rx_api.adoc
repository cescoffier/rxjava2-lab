...
Now that you are experts in RX, let's see how you can create an RX API for your users. First, what do we call _RX API_. A _RX API_ is an API using, and generally returning RX Java types such as `Single`, `Completable` or `Flowable`. This API is made to be asynchronous. Building such an API has several benefits:

* it gives to the user the ability to use the RX operator and call your API that way - it's a great way to handle asynchrony. 
* it avoids you to re-implement structures to handle streams or async types - because don't be mistaken, asynchrony is hard.

When building such an API there are several cases and questions you need to answer:

1. which types for which method - what RX type do you provide or consume
...
| Operation returning multiple results produces externally | `x(Listener<T> listener)` | `Observable<T> x()` | `clicks()`, `keystrokes()`
...
In this chapter we are going to create several versions of an API using the Super heroes and villains service. Unlike the one you used before, this one will retrieve the content directly from the file system. The API is defined in `me.escoffier.lab.chapter5.SuperAPI`:
...
Let's have a look at the https://vertx.io/docs/vertx-core/java/#_using_the_file_system_with_vert_x[Vert.x FileSystem API] and it's _rx-ified_
https://vertx.io/docs/apidocs/io/vertx/reactivex/core/file/FileSystem.html[version]. In `me.escoffier.lab.chapter5.Code1`, we read the full set of super heroes and villains:
...
As you can see it's just a matter of building the right pipeline to get the file content and transform it the result we are looking for. For instance, `me.escoffier.lab.chapter5.Code2` creates a `JsonArray` from the content of the file:
...
Notice the usage of the `cast` operator that just check that the item can be cast to the given class and _cast_ it.

==== Implementing the heroes and villains methods

So as you have seen above, when you have a RX API, providing your API on top of is just about providing the right pipeline. Let's start with the easy ones: `villains` and `heroes`. Open the `me.escoffier.lab.chapter5.Code4` class and fill the code to returns the stream of heroes and villains. Use the `Character.isVillain` method as predicates to know if the character is on the good side or bad side.
...
Let's increase a bit the difficulty with the `hero` and `villain` API that return a _random_ hero or villain. Open the `me.escoffier.lab.chapter5.Code5` and edit the code to implement the 2 methods. The solution proposes 2 different approaches.
...
There are 2 `findByName` methods. Both takes a non-null name as parameter. The `findByName` returns a `Maybe`, empty if no character matches. The `findByNameOrError` returns a `Single`. If the lookup failed an error is emitted in the stream.
...
We have implemented the first variant of our API. Because we have a RX API as input, the implementation is straightforward. Let's now look at more complicated cases.
...
https://docs.oracle.com/javase/8/docs/api/java/nio/channels/AsynchronousFileChannel.html[`AsynchronousFileChannel`] from the JDK, for example: they allow you to read a file asynchronously, and require a listener from you, which will be notified on success or failure. 
...
http://reactivex.io/RxJava/javadoc/io/reactivex/SingleEmitter.html[`Emitter`] object that you use to push notifications into the `Single` you're creating (`/Users/clement/Desktop/rxjava2-lab/src/main/java/me/escoffier/lab/chapter5/Code7.java`): 
...
We are now in the worse case. We need to implement our API but we can't use neither a base RX-API or an asynchronous API. Like for creating `Single`, it is possible to create the other types `Flowable`, `Observable`, `Maybe` and `Completable` from scratch with an `Emitter`.

In the next example, we're going to use  http://reactivex.io/RxJava/javadoc/io/reactivex/Observable.html#create-io.reactivex.ObservableOnSubscribe-[`Observable.create`] to push all the files (recursively) found in the specific directory.
...
Using threads like this is not recommended. But you need to be aware of the possibility. The last set of example have illustrated issues when dealing with blocking operations. Fortunately, RX proposes various solutions to handle this case. This is covered in the next chapter.
