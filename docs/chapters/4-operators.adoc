...
`me.escoffier.lab.chapter4.Code3` gives an example of `defaultIfEmpty`. It basically injects a default value if the observed stream is empty.
...
The `zipWith` operator associates items from different streams. It takes the _next_ items emitted by each stream and calls a function with all of them. For example, we can use this to generate super heroes vs. super villains battles. Open the `me.escoffier.lab.chapter4.Code13` class and fill the code to generate fights between heroes and villains.
...
Even when using RX, error happens. I told you it's not a silver bullet. But, I've a good news for you: there are operators to handle error recovery. 

Let's start with some _simple" case. Image you want to emit a request to our Super Heroes and Villains service, but the service is not started. Something like the `me.escoffier.lab.chapter4.Code14` class:
...
The `onErrorResumeNext` operator is similar to `onErrorReturnItem` but returns a stream. Basically, if an error happen, it replaces the stream with the given one. Open the `me.escoffier.lab.chapter4.Code15` class and use the `onErrorResumeNext` to inject another `Single`. You can use `Single.just` to create a stream with an already known value.
...
The last error recovery mechanism we are going to see is very powerful but can be dangerous too. The `retry` operator re-subscribes to a stream that emitted an error. It's behavior depends on the cold / hot nature of the observed stream. On a cold stream, the resubscription restarts the stream from the beginning. To it's very useful to retry a failed operation such as a HTTP call. The retry operator has several variant letting you decide whether or not you want to retry based on the error and the number of attempts.
