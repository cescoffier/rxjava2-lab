...
Now let's see a couple of examples where we can directly apply schedulers to control the concurrency of our API. In the first exercise we use the `java.net.HttpURLConnection` to achieve an HTTP request to the _Super Heroes Service_. `HttpURLConnection` is certainly one of the worst way to do an HTTP request but it's a good example of an API that blocks waiting for IO completions. In `me.escoffier.lab.chapter6.Code7`, use the `subscribeOn`, to change the thread performing the HTTP requests. As it's clearly an io task, use the `Schedulers.io()` scheduler.
