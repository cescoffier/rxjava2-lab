...
The `create` method takes a method in parameter called on every subscription (so for every subscriber). The following code shows how this method is used:
...
Once cancelled, the subscriber does not receive any event anymore. It does not get a completion event either.
