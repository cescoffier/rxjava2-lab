...
Singles are often used for asynchronous operation returning a single result, such as an HTTP request. The following example use the Vert.x Web Client to retrieve a list of super heroes:
...
Maybes are often used for method that may return `null`. For example, an asynchronous version of a `findById` method would return a `Maybe`. Let's use maybe to check if there is a super heroes named "Yoda" and another one named "Clement". Open the `me.escoffier.lab.chapter3.Code5` class and fill the code. The output should be something like:

[source]
----
Loaded 727 heroes and villains
Yes, Yoda is a super hero
No, clement is not a super hero
----
...
In `me.escoffier.lab.chapter3.Code7`, fill the code to write a simple message into a file. For this use the method named `rxWriteFile` that accept the path (such as `hello.txt`) and a `Buffer` (such as `Buffer.buffer("hello")`). It returns a `Completable` indicating when the file has been written (and it the write has been successful).
...
Running this example produce an output similar to the following one:
...
We have constructed many many items, while the subscriber is only processing the item 95! The emissions of the number are too fast for the consumer, and because the emissions are being pushed in  an unbounded buffer by `observeOn`, this can be the source of many problems such as... out of memory.

To mitigate this issue, RX Java 2 provides a stream named `Flowable`. `Flowable` is like `Observable` (so may contain multiple items), but implement a back-pressure protocol. This protocol tells the source stream to emit items at a pace specified by the consumer. `Flowable` use a protocol named http://www.reactive-streams.org/[Reactive Streams]. This specification has been introduced in Java 9 under the name `java.util.concurrent.Flow` and is becoming widely popular.
...
What we can see here is that the source emits a set of items (128) and then, 96 items have been processed by the rest of the flow. During that time, no items have been emitted. So the consumer is telling to the source that it can't handle more at that time, and the source stops emitting. When the consumer can finally handle more items, it requests more to the source. So no more risk of OOM!

Notice that RX JAva 2 provides some other back pressure strategies such as using buffer, or dropping data. Check the https://github.com/ReactiveX/RxJava/wiki/Backpressure-(2.0)[documentation] for further details on these strategies.

== Conclusion

In this chapter we have seen a few different specialized streams to handle various situation. The following table summarizes when to use what:
